...
const localAdminUrl = (config) => `http://${config.functionHostname}:${config.functionPort ||
    defaultFunctionPort}/admin/functions/`;
const localFunctionUrl = (config) => `http://${config.functionHostname}:${config.functionPort ||
...
exports.triggerFunction = async (config, hub, consumerGroup) => {
    ...
    try {
        ...
        while (!func) {
            try {
                console.log(`Retrieving function info from ${colors_1.green(config.functionHostname)}:${colors_1.green((config.functionPort || defaultFunctionPort).toString())}`);
